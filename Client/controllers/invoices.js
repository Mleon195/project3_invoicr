...
myApp.controller('InvoicesController', ['$scope', '$http','$location','$routeParams', function($scope, $http,$location, $routeParams){
	...
	$scope.getInvoice = function(){
		...
		$http.get('/api/invoices/'+id).success(function(response){
			...
			//Fill Select
			// $scope.invoice.customer_id = response.customer;
      // $http.get('/api/customers/'+$scope.invoice.customer._id).success(function(response){
      //   $scope.customer = response;
      // });
      response.status = $scope.invoice.status;
		});
	};
